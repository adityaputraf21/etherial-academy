...
import { Shield, LayoutDashboard, BookCopy, UserCheck, UserCircle, LogOut, Menu, X } from 'lucide-react';

export function TutorDashboardLayout({ user, onLogout, onNavigate, activeView, children }) {
    const [sidebarOpen, setSidebarOpen] = useState(false);

    const navItems = [
        { name: 'Dashboard', icon: LayoutDashboard, view: 'home' },
        { name: 'Kelas Saya', icon: BookCopy, view: 'my-classes' },
        { name: 'Absensi Siswa', icon: UserCheck, view: 'attendance' },
    ...
    const SidebarContent = () => (
        <div className="flex flex-col h-full bg-slate-800">
            <div className="flex items-center justify-center h-20 border-b border-slate-700 flex-shrink-0">
                ...
                <h1 className="text-xl font-bold text-white font-serif tracking-wider">TUTOR PORTAL</h1>
            </div>
            ...
            <div className="flex-1 flex flex-col max-h-screen">
                <header className="flex-shrink-0 bg-slate-800/80 backdrop-blur-lg md:bg-transparent p-4 flex justify-between items-center md:justify-end border-b md:border-none border-slate-700">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <img src={`https://placehold.co/40x40/8b5cf6/ffffff?text=${user.fullName.charAt(0)}`} alt="Avatar" className="w-10 h-10 rounded-full" />
                    </div>
                </header>
